...
class AvailabilityService {
    ...
    generateTimeSlots(startDate, endDate, durationMinutes, intervalMinutes, businessHours, calendars, originalTimezone) {
        ...
        while (currentDate <= endDate) {
            ...
            if (daySchedule && daySchedule.enabled) {
                ...
                const morningStart = luxon_1.DateTime.max(rawMorningStart, startDate);
                const morningEnd = luxon_1.DateTime.min(rawMorningEnd, endDate);
                ...
                if (daySchedule.break_start && daySchedule.break_end) {
                    ...
                    const afternoonStart = luxon_1.DateTime.max(rawAfternoonStart, startDate);
                    const afternoonEnd = luxon_1.DateTime.min(rawAfternoonEnd, endDate);
     ...
     */
    addSlotsForPeriod(periodStart, periodEnd, durationMinutes, intervalMinutes, calendars, slots) {
    ...
    applyTimeBlocksStrategy(slots, maxResults, config) {
        ...
        const eveningSlots = [];
        slots.forEach(slot => {
            const slotTime = luxon_1.DateTime.fromISO(slot.start_datetime);
    ...
    applyBalancedDistribution(slots, maxResults) {
        ...
        slots.forEach(slot => {
            const day = luxon_1.DateTime.fromISO(slot.start_datetime).toFormat('yyyy-MM-dd');
