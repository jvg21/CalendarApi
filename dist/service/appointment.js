...
class AppointmentService {
    constructor() {
        this.availabilityService = new availability_1.AvailabilityService();
    }
    async createAppointment(request) {
        const { instance_id, service_id, start_datetime, end_datetime, client_name, client_email, client_phone, description, calendar_id, flow_id, agent_id, user_id } = request;
        // Get service details
        ...
        const startTime = luxon_1.DateTime.fromISO(start_datetime, { setZone: true });
        const endTime = end_datetime
            ? luxon_1.DateTime.fromISO(end_datetime, { setZone: true })
        ...
        const availabilityCheck = await this.availabilityService.checkAvailability(start_datetime, service_id, targetCalendar.id);
        ...
        try {
            ...
            if (error) {
                ...
                await this.deleteEventFromSharedCalendar(targetCalendar.google_calendar_id, googleEvent.id);
    ...
    async updateAppointment(id, updates) {
        const { data: appointment } = await supabase_1.supabase
            .from('appointments')
        ...
        if (updates.start_datetime && !updates.end_datetime) {
            const startTime = luxon_1.DateTime.fromISO(updates.start_datetime, { setZone: true });
        ...
        if (updates.start_datetime || updates.end_datetime) {
            ...
            const availabilityCheck = await this.availabilityService.checkAvailability(newStartTime, appointment.service_id, appointment.calendar_id);
            if (!availabilityCheck.available) {
        ...
        if (updates.start_datetime || updates.end_datetime || updates.title) {
            ...
            const eventStartTime = updates.start_datetime
                ? luxon_1.DateTime.fromISO(updates.start_datetime, { setZone: true })
                : luxon_1.DateTime.fromISO(appointment.start_datetime, { setZone: true });
            const eventEndTime = endTime
                ? luxon_1.DateTime.fromISO(endTime, { setZone: true })
                : luxon_1.DateTime.fromISO(appointment.end_datetime, { setZone: true });
            await this.updateEventInSharedCalendar(appointment.calendars.google_calendar_id, appointment.google_event_id, {
        ...
        const { data: updatedAppointment, error } = await supabase_1.supabase
            .from('appointments')
            .update(updateData)
            .eq('id', id)
            .select()
    ...
    async deleteAppointment(id) {
        ...
        if (appointment.google_event_id && appointment.calendars?.google_calendar_id) {
            await this.deleteEventFromSharedCalendar(appointment.calendars.google_calendar_id, appointment.google_event_id);
        }
    ...
    async validateExistingAppointment(appointmentId) {
        const { data: appointment } = await supabase_1.supabase
            .from('appointments')
            .select('*, calendars(*)')
            .eq('id', appointmentId)
        ...
        const availabilityCheck = await this.availabilityService.checkAvailability(appointment.start_datetime, appointment.service_id, appointment.calendar_id);
        return availabilityCheck.available;
    }
    // 🔧 CORREÇÃO: Função reformulada para trabalhar com DateTime
    async createEventInSharedCalendar(eventData) {
        const event = {
        ...
        const response = await google_calendar_1.calendar.events.insert({
            calendarId: eventData.targetCalendarId,
            requestBody: event,
    ...
    // 🔧 CORREÇÃO: Função reformulada para trabalhar com DateTime
    async updateEventInSharedCalendar(calendarId, eventId, updates) {
        const event = {
        ...
        await google_calendar_1.calendar.events.update({
            calendarId,
            eventId,
            requestBody: event,
    ...
    async deleteEventFromSharedCalendar(calendarId, eventId) {
        try {
            await google_calendar_1.calendar.events.delete({
                calendarId,
                eventId,
    ...
    buildEventDescription(service, clientName, description) {
    ...
    async checkSharedCalendarAvailability(calendarId, startTime, endTime) {
        try {
    ...
    async getSharedCalendarEvents(calendarId, startDate, endDate) {
        try {
            const response = await google_calendar_1.calendar.events.list({
                calendarId,
    ...
    async updateExpiredAppointments(filters) {
        try {
            ...
            const { data: updatedAppointments, error: updateError } = await supabase_1.supabase
                .from('appointments')
                .update({
                status: 'completed',
                updated_at: new Date().toISOString()
