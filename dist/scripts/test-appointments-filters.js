...
async function testAppointmentsFilters() {
    ...
    try {
        ...
        const { data: allAppointments, error: allError } = await supabase_1.supabase
            .from('appointments')
            .select(`
        *,
        instances(name),
      ...
      `)
        ...
        if (allAppointments && allAppointments.length > 0) {
            ...
            if (testInstanceId) {
                console.log('\n📋 2. Teste filtro por instance_id');
                const { data: instanceFiltered, error: instanceError } = await supabase_1.supabase
                    .from('appointments')
            ...
            if (testStatus) {
                console.log('\n📋 3. Teste filtro por status');
                const { data: statusFiltered, error: statusError } = await supabase_1.supabase
                    .from('appointments')
            ...
            console.log('\n📋 4. Teste filtro por data');
            const { data: dateFiltered, error: dateError } = await supabase_1.supabase
                .from('appointments')
            ...
            if (testFlowId) {
                ...
                const { data: flowFiltered, error: flowError } = await supabase_1.supabase
                    .from('appointments')
            ...
            if (testAgentId) {
                ...
                const { data: agentFiltered, error: agentError } = await supabase_1.supabase
                    .from('appointments')
            ...
            if (testUserId) {
                ...
                const { data: userFiltered, error: userError } = await supabase_1.supabase
                    .from('appointments')
            ...
            const { data: combinedFiltered, error: combinedError } = await supabase_1.supabase
                .from('appointments')
