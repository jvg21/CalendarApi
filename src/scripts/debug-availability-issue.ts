...
async function debugAvailabilityIssue() {
  ...
  try {
    ...
    const searchStart = DateTime.fromISO(testParams.start_datetime);
    const searchEnd = DateTime.fromISO(testParams.end_datetime);
    ...
    const suggestions = await availabilityService.suggestAvailability(
      testParams.start_datetime,
      testParams.end_datetime,
      testParams.service_id,
      testParams.calendar_ids,
      testParams.max_results,
      testParams.expand_timeframe,
      testParams.interval_minutes,
      testParams.strategy
    );
    ...
    suggestions.forEach((suggestion, index) => {
      ...
      const suggestionStart = DateTime.fromISO(suggestion.start_datetime);
      const suggestionEnd = DateTime.fromISO(suggestion.end_datetime);
    ...
    for (const cal of calendars) {
      ...
      const startTime = DateTime.fromISO("2025-08-26T14:00:00.000-03:00");
      ...
      try {
        const directResponse = await calendar.events.list({
          calendarId: cal.google_calendar_id,
    ...
    const conflictingSuggestions: any[] = [];
    ...
    conflictingSuggestions.forEach((conflict, index) => {
      ...
      conflict.conflicts.forEach((event: any) => {
        console.log(`     - ${event.summary || 'Sem título'}`);
...
async function debugSpecificSlot() {
  ...
  try {
    ...
    const startTime = DateTime.fromISO(TEST_TIME, { setZone: true });
    const endTime = startTime.plus({ minutes: service.duration });
    const totalStart = startTime.minus({ minutes: service.buffer_before || 0 });
    const totalEnd = endTime.plus({ minutes: service.buffer_after || 0 });
