...
async function quickTest() {
  ...
  try {
    ...
    for (let i = 0; i < Math.min(suggestions.length, 5); i++) {
      ...
      const slotStart = DateTime.fromISO(suggestion.start_datetime);
      const slotEnd = DateTime.fromISO(suggestion.end_datetime);
    ...
    if (problemSlots.length > 0) {
      ...
      problemSlots.forEach((problem, index) => {
        ...
        problem.google_conflicts.forEach((conflict: any) => {
          console.log(`     - Google: ${conflict.summary} (${conflict.start} to ${conflict.end})`);
        });
        
        problem.db_conflicts.forEach((conflict: any) => {
    ...
    for (const calendarId of testData.calendar_ids) {
      ...
      const individualResult = await availabilityService.checkAvailability(
